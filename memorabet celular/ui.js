import { K_MAX, TOTAL_PAIRS, C } from './constants.js?v=69';
import { gameState, session } from './state.js?v=69';
import { escapeHTML, formatMoney } from './utils.js?v=69';
import { updateSaldo, updateUserAvatar, updateUserCardSkins } from './database.js?v=69';
...
export function updateStats(){
  ...
  if(ganancia){
    ganancia.textContent = localDuel ? (onlineDuel ? 'Online' : 'Offline') : formatMoney(gameState.saldo);
    ganancia.className = localDuel ? 'success' : (gameState.saldo >= C ? 'success' : 'danger');
  }
...
export function showOnlineVictoryAnimation({ winnerName = 'Jugador', reason = 'Partida online terminada.', autoCloseMs = 3200 } = {}){
  const old = document.getElementById('victory-overlay');
  if(old) old.remove();
