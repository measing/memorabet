import { ANIMAL_CARDS, K_MAX, TOTAL_PAIRS, G, C, INITIAL_SALDO } from './constants.js?v=69';
import { createLocalDuelState, gameState, session, resetGameState } from './state.js?v=69';
import { shuffle, wait, formatMoney } from './utils.js?v=69';
import {
  updateSaldo,
  resetOnlineStats,
  addLiveHistory,
  ...
  updateOnlineRoom,
  removeOnlineRoom
} from './database.js?v=69';
import { renderBoard, updateCardClasses, updateStats, showMsg, hideMsg, clearBoard, renderUserStats, setNewGameButtonBusy, showVictoryAnimation, showOnlineVictoryAnimation, formatDuration, getSelectedAvatar } from './ui.js?v=69';
import { playCardFlip, playShuffle, playMatch, playMiss, playRivalFound } from './audio.js?v=69';
  ...
  return new Promise(resolve => {
    ...
    overlay.innerHTML = `
      <div class="online-exit-box">
        ...
        <p>Si sales ahora, el otro jugador gana automaticamente.</p>
...
function applyOnlineRoom(room){
  ...
  if(room.status === 'finished' && handledOnlineFinishId !== room.id){
...
export async function startOnlineGame(mode = 'classic'){
  ...
  gameState.intentos = 0;
  gameState.gananciaPartida = 0;
  gameState.onlineRoom = { mode, status:'searching' };
  gameState.localDuel = createLocalDuelState();
  gameState.localDuel.active = true;
  ...
  gameState.localDuel.statusText = 'Buscando rival online...';
  setNewGameButtonBusy(true);
  clearBoard();
  updateStats();
  ...
  try{
    ...
    const waitingRoom = await findWaitingOnlineRoom(mode, player.uid);
    const room = waitingRoom
      ? await joinOnlineRoom(waitingRoom.id, player)
      : await createOnlineRoom(mode, player);
    activeOnlineRoom = room;
    gameState.onlineRoom = { id:room.id, mode:room.mode, status:room.status };
...
export async function resetGame(){
  ...
  setNewGameButtonBusy(false);
  clearBoard();
  if(session.currentUser && !isGuestUser()) await resetOnlineStats(session.currentUser.uid);
  updateStats();
